...
function parsePredicate(state) {
    return lift(state)
        .then(parseExp)
        .then(oftype(ast.predicate.PredicateNode));
}
...
function parseExp$(state) {
    return lift(state)
        .then(match(tags.or))
        .then(parseExp)
        .then(stack.popPushNode(2, ast.predicate.OrNode));
}
...
function parseExp1$(state) {
    return lift(state)
        .then(match(tags.and))
        .then(parseExp1)
        .then(stack.popPushNode(2, ast.predicate.AndNode));
}
...
function parseExp2$eq(state) {
    return lift(state)
        .then(match(tags.eq))
        .then(parseExp2)
        .then(stack.popPushNode(2, ast.predicate.EqNode));
}
function parseExp2$ne(state) {
    return lift(state)
        ...
        .then(parseExp2)
        .then(stack.popPushNode(2, ast.predicate.EqNode))
        .then(stack.popPushNode(1, ast.predicate.NotNode));
}
...
function parseExp3$lt(state) {
    return lift(state)
        .then(match(tags.lt))
        .then(parseExp3)
        .then(stack.popPushNode(2, ast.predicate.LtNode));
}
function parseExp3$lte(state) {
    return lift(state)
        .then(match(tags.lte))
        .then(parseExp3)
        .then(stack.popPushNode(2, ast.predicate.LteNode));
}
function parseExp3$gte(state) {
    return lift(state)
        .then(match(tags.gte))
        .then(parseExp3)
        .then(stack.popPushNode(2, ast.predicate.GteNode));
}
function parseExp3$gt(state) {
    return lift(state)
        .then(match(tags.gt))
        .then(parseExp3)
        .then(stack.popPushNode(2, ast.predicate.GtNode));
}
...
function parseExp4$plus(state) {
    return lift(state)
        ...
        .then(parseExp4)
        .then(stack.popPushNode(2, ast.numeric.AdditionNode));
}
function parseExp4$minus(state) {
    return lift(state)
        .then(match(tags.minus))
        .then(parseExp4)
        .then(stack.popPushNode(2, ast.numeric.SubtractionNode));
}
...
function parseExp5$times(state) {
    return lift(state)
        ...
        .then(parseExp5)
        .then(stack.popPushNode(2, ast.numeric.MultiplicationNode));
}
function parseExp5$div(state) {
    return lift(state)
        .then(match(tags.div))
        .then(parseExp5)
        .then(stack.popPushNode(2, ast.numeric.DivisionNode));
}
...
function parseExp7$count(state) {
    return lift(state)
        ...
        .then(stack.popPushNode(1, ast.numeric.CountNode));
}
...
function parseExp7$exists(state) {
    return lift(state)
        ...
        .then(stack.popPushNode(1, ast.predicate.PathExistsNode));
}
...
function parseExp7$unique(state) {
    return lift(state)
        .then(match(tags.unique))
        .then(stack.popPushNode(1, ast.predicate.PathUniqueNode));
}
